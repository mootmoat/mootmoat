...
description: Learn how to integrate Solstice with an existing template by Cosmic Themes
sidebar:
  order: 3
...
2. Import the docs styles into your global css file.
    ...
    ```
3. Add the docs content collection and tabs setup to your site's content collection config file.

    ```ts title="src/content.config.ts" ins={3-18, 22-52, 56}
    // other imports...
...
5. Add the docs pages to your site. 
    ...
    From the Solstice theme copy the whole `src/pages/[docsRoute]` folder into your site's `src/pages/[docsRoute]` folder.
...
This theme uses the **Geist** and **Geist Mono** fonts by default. It is recommended to install the fonts locally from fontsource [Geist](https://fontsource.org/fonts/geist/install) and [Geist Mono](https://fontsource.org/fonts/geist-mono/install).
